...
<title>${student.firstName} ${student.lastName}</title>
		...
		<div class="row">
			<div class="col col-md-6">
				<form:form name='f' action='${pageContext.request.contextPath}/updateStudent/?old_group_id=${student.groupID}' method='POST' modelAttribute="student">
					...
					<form:select path="birthYear" name='birthYear' class="form-control" id="birthYear">
					    <c:forEach begin="1900" end="2020" varStatus="loop">
					    	<c:choose>
		    					<c:when test="${student.birthYear == loop.index}">
				    ...
				    <form:select path="groupID" name='groupID' class="form-control" id="groupSelect">
					    ...
					    <c:forEach var="group" items="${groups}">
					      	<c:choose>
		    					<c:when test="${student.groupID == group.groupID}">
